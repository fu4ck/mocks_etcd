...
func NewEtcdCustomClient(ctx context.Context, addr string, timeout time.Duration) (*EtcdClient, error) {
	...
	pool, err := NewRoundRobinEtcdClientAllocator([]string{addr}, timeout, capacity, maxUsage)
	if err != nil {
...
func NewEtcdClient(ctx context.Context, addr string, timeout time.Duration) (*EtcdClient, error) {
	return NewEtcdCustomClient(ctx, addr, timeout)
}
